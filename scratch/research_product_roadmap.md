...
/* QA FEEDBACK:
CRITICAL ISSUES DETECTED: FAILThe roadmap contains logic flaws and direct violations of the non-negotiable AGENTS.md rules. You must fix the following issues before this file can pass:1. Direct Rule Violations (CRITICAL)Capacitor Config Tampering: Under Stack: Capacitor vs React Native, the text instructs to: "Change to: com.face2face.app / "Face 2 Face"".Violation: Section 1 of AGENTS.md explicitly states: capacitor.config.ts (changing appId is permanent) is a NEVER modify file without explicit user approval. An autonomous agent reading this roadmap might try to automate this change and brick the configuration.Fix: Remove the instruction or flag it explicitly as [REQUIRES MANUAL USER INTERVENTION - DO NOT AUTOMATE].2. Broken Logic & Feature ContradictionsThe "Map Games" Paradox:Under CURRENT STATE (Already Built), you list Location-based games (8 games!) as ✅ Built.Under v1.0 MVP — Day 1 Launch / Cut from v1, you list All 8 Map Games (re-enable in v1.5).Under v1.5 — Month 2-3, you list Re-enable Map Games with an effort of Already built.The Flaw: If they are already built and functioning, cutting them from the MVP deployment package to "ship faster" makes no sense unless they are actively buggy or breaking the build. If they are broken, they are not "Already Built (✅)".Fix: Clarify the status. If they are built but hidden behind a feature flag, label them as Feature Flagged (Disabled for Launch) instead of "Cut from v1".Push Notification Status Clash:CURRENT STATE claims: Push notification tokens (Capacitor ready) ✅ Built.Retention Impact Ranking claims: 1. Push Notifications (#1 retention lever) is missing/needs implementation.v1.5 table allocates 3-5 days of effort to build them.The Flaw: Token registration is not a full push notification system.Fix: Change the Current State line to Push token registration schema ready (Backend/Frontend plumbing missing).3. Typographical ErrorsSpacing Error: In the Retention Impact Ranking section, item 4 reads (saf ety = retention). Fix the typo to (safety = retention).Status: FAIL
Fix the contradictions and explicit rule-breaking guidelines listed above.
